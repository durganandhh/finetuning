...
# -------------------------------
# Stage 0: Feature Extraction (hires)
# -------------------------------
if [ $stage -le 0 ]; then
  echo "=== Stage 0: Extracting high-resolution MFCCs and i-vectors ==="
  for x in $train_set $dev_set; do
    utils/fix_data_dir.sh data/$x

    # Make hires MFCCs
    steps/make_mfcc.sh --nj 10 --mfcc-config conf/mfcc_hires.conf \
      data/speechocean762_train data/speechocean762_train_hires mfcc
    steps/compute_cmvn_stats.sh data/speechocean762_train_hires


    # Extract i-vectors
    steps/online/nnet2/extract_ivectors_online.sh --nj $nj \
      data/${x}_hires $ivector_extractor tune/nnet3_cleaned/ivectors_$x
  done
fi
...
if [ $stage -le 1 ]; then
  ...
  if [ ! -d $align_dir ]; then
    steps/nnet3/align.sh --cmd "run.pl" --nj $nj --use-gpu true \
      ...
      data/${train_set}_hires $lang $pretrained_model $align_dir
  else
    echo "Alignments already exist in $align_dir, skipping alignment."
  fi
fi
...
if [ $stage -le 2 ]; then
  ...
  steps/nnet3/align_lats.sh --cmd "run.pl" --nj $nj \
    --online-ivector-dir tune/nnet3_cleaned/ivectors_${train_set} \
    data/${train_set}_hires $lang $pretrained_model $lat_dir
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/nnet3/chain/get_egs.sh --cmd "run.pl" \
...
# -------------------------------
if [ $stage -le 5 ]; then
  echo "=== Stage 5: Fine-tuning completed successfully ==="
