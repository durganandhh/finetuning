...
for entry in phoneme_comparisons:
    print(f"{entry['utterance_id']} | {entry['phoneme_index']} | {entry['word']} | "
          f"Human={entry['human_phoneme']} | Predicted={entry['predicted_phoneme']} | "
          f"HumanScore={entry['human_score']} | PredictedScore={entry['predicted_score']}")

# Calculate Mean Squared Error (MSE)
human_score_list = [entry["human_score"] for entry in phoneme_comparisons]
predicted_score_list = [entry["predicted_score"] for entry in phoneme_comparisons]

if human_score_list:
    mse = sum((h - p) ** 2 for h, p in zip(human_score_list, predicted_score_list)) / len(human_score_list)
    print(f"\nMean Squared Error (MSE): {mse:.4f}")
else:
    print("\nNo valid phoneme comparisons found to compute MSE.")
