...
with open(OUTPUT_CSV, "w", newline="") as csvfile:
    ...
    for utt_id, gop_vals in utt_gop.items():
        ...
        # Handle utterances with all correct or all mispronounced phones
        if np.all(y_true == 0):
            eer_str = "All Correct"
            eer_threshold_gop = "NA"
        elif np.all(y_true == 1):
            eer_str = "All Mispronounced"
            eer_threshold_gop = "NA"
        else:
            # Normal EER computation
            fpr, tpr, thresholds = roc_curve(y_true, -y_score)
            fnr = 1 - tpr
            eer_idx = np.nanargmin(np.abs(fnr - fpr))
            eer = fpr[eer_idx]
            eer_threshold_gop = -thresholds[eer_idx]
            eer_str = round(eer*100, 2)
            all_eers.append(eer)

        writer.writerow([utt_id, min_len, eer_str,
                         round(eer_threshold_gop,3) if isinstance(eer_threshold_gop,float) else eer_threshold_gop])
...
if all_eers:
    ...
    print("No valid utterances found for numeric EER computation.")
